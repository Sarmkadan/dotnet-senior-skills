---
name: http-resilience-and-outbound-calls
description: Review outbound HTTP in .NET - IHttpClientFactory usage, timeouts, retries with idempotency awareness, circuit breakers, and response handling. Use when reviewing HttpClient code, Polly/resilience policies, or any service-to-service calls.
---
...
`Microsoft.Extensions.Http.Resilience` (`AddStandardResilienceHandler()`) or Polly - never a hand-rolled `for` loop with `Task.Delay`.

```csharp
services.AddHttpClient<CatalogClient>(...)
    .AddStandardResilienceHandler(); // retry + circuit breaker + per-attempt timeout, sane defaults
```
...
- Retry on: 408, 429 (honor `Retry-After`), 5xx, connection failures. Never on 4xx besides those - retrying a 400 three times just burns latency on a request that cannot succeed.
